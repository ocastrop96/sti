...
class TablaIntegraEP
{
    public function mostrarTablaIntegraEP()
    {
        ...
        for ($i = 0; $i < count($integracion); $i++) {
            $botones = "<div class='btn-group'><button class='btn btn-warning btnEditarIntegraEP' idTipo='" . $integracion[$i]["tipo_equipo"] . "' idIntegracion='" . $integracion[$i]["idIntegracion"] . "' data-toggle='modal' data-target='#modal-editar-integraEP'><i class='fas fa-edit'></i></button><button class='btn btn-success btnVerFichaEP' idIntegracion='" . $integracion[$i]["idIntegracion"] . "'><i class='fas fa-eye'></i></button><button class='btn btn-info btnImprimirFichaEP' idIntegracion='" . $integracion[$i]["idIntegracion"] . "'><i class='fas fa-print'></i></button><button class='btn btn-secondary btnAnularIntegraEP' idIntegracion='" . $integracion[$i]["idIntegracion"] . "'><i class='fas fa-window-close'></i></button></div>";
            $datos_json .= '[
                "' . ($i + 1) . '",
