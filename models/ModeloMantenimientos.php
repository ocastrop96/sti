...
class ModeloMantenimientos
{
    ...
    {
        $stmt = Conexion::conectar()->prepare("CALL REGISTRAR_MANTENIMIENTO(:fRegistroMant,:descInc,:tipEquipo,:condInicial,:idEquip,:oficEquip,:areaEquip,:respoEquip,:tecEvalua,:tipTrabajo,:fEvalua,:primera_eval,:fInicio,:fFin,:recomendaciones,:estAtencion,:condFinal,:servTerce,:otros,:obsOtros,:usRegistra,:sgmtoManto,:logdeta,:diagnosticos,:acciones)");
    ...
    static public function mdlActualizarDiagnostico($dato)
    {
        $stmt = Conexion::conectar()->prepare("CALL ACTUALIZAR_CONT_DIAG1(:idDiagnostico)");
    ...
    static public function mdlActualizarDiagnostico2($dato)
    {

        $stmt = Conexion::conectar()->prepare("CALL ACTUALIZAR_CONT_DIAG2(:idDiagnostico)");
        $stmt->bindParam(":idDiagnostico", $dato, PDO::PARAM_STR);
        $stmt->execute();
        return $stmt->fetch();
        $stmt->close();
    ...
    static public function mdlActualizarAccion($dato)
    {
        $stmt = Conexion::conectar()->prepare("CALL ACTUALIZAR_CONT_ACCI1(:idAccion)");
        $stmt->bindParam(":idAccion", $dato, PDO::PARAM_STR);
        $stmt->execute();
        return $stmt->fetch();
        $stmt->close();
        $stmt = null;
    }
    static public function mdlActualizarAccion2($dato)
    {
        $stmt = Conexion::conectar()->prepare("CALL ACTUALIZAR_CONT_ACCI2(:idAccion)");
        $stmt->bindParam(":idAccion", $dato, PDO::PARAM_STR);
        $stmt->execute();
        return $stmt->fetch();
        $stmt->close();
